...
function main() {
	...
	for (const gridImage of gridImages) {
		gridImage.addEventListener('click', (event) => {
			...
			// Set board name.
			const imagePopupBoardElem = document.getElementById(
				'img-popup-board-name'
			);
	...
	try {
		...
		console.log('Error initializing slideshow:', e);
	}
...
// Initialize the slideshow with pins data
function initializeSlideshow() {
	// Get all the pins from the grid
	const gridItems = document.querySelectorAll('.grid-item');
	if (gridItems.length === 0) {
		console.log('No pins available for slideshow');
		return;
	}

	// Extract pin data from the grid items
	const pins = Array.from(gridItems).map((item) => {
		const img = item.querySelector('.grid-image');
		const boardBadge = item.querySelector('.pin-badge span');

		return {
			imageURL: img.getAttribute('src'),
			id: img.getAttribute('pin-url').split('/').pop(),
			board: {
				name: boardBadge
					? boardBadge.textContent
					: img.getAttribute('board-name'),
			},
		};
	});
	...
	slideshow = new PinSlideshow(pins, slideshowContainer);

	// Add event listener to the slideshow button
...
function applySettings(r, settings) {
	...
	r.style.setProperty('--img-filter', filter);
}
